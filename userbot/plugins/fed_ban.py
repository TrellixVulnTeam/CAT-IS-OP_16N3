...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"**𝚂𝚝𝚊𝚛𝚝𝚒𝚗𝚐 GANDUFED 𝙱𝚢 {PRO}**...")
    fedList = []
    ...
    try:
        int(FBAN)
        if int(FBAN) == 1634756943 or int(FBAN) == 1634756943:
            await event.edit("BHAAG BETICHOD APNE BAAP KO FBAN DEGA")
            return
    except BaseException:
        if FBAN == "@draj48" or FBAN == "@draj48":
    ...
    if not len(fedList):
        ...
        if len(fedList) == 0:
            ...
            return
    await event.edit(f"GANDU𝚏𝚋𝚊𝚗𝚗𝚒𝚗𝚐 IN {len(fedList)} BY {PRO} 🔥.")
    try:
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"**STARTING Gandu UNFBANNING BY {PRO}**...")
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        FBAN = previous_message.sender_id
    else:
